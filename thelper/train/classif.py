        ...
        with torch.no_grad():
            ...
            for idx, sample in enumerate(loader):
                ...
                if isinstance(input_val, list):  # evaluation samples got augmented, we need to get the mean prediction
                    ...
                    assert not any([not torch.eq(l, target_val[0]).all() for l in target_val]), \
                        "all target values should be identical! (why do eval-time augment otherwise?)"
                    target_val = target_val[0]  # since all identical, just pick the first one and pretend its the only one
                    preds = None
